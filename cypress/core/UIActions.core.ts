...
import { errorLog, logAction } from "./logs.core";
...
export class UIActions {
 ...
 /**
   ...
   * click(loginBtn);
   */
 public click(control: WebControl) {
    try {
        cy.get(control.selector).first().click();
...
public async type(control: WebControl, textTobeEntered: string) {
    try{
        cy.get(control.selector).first().type(textTobeEntered);
        logAction("Entered " +textTobeEntered + " value in "+control.controlDescription);
    }catch (e) {
        errorLog("Unable to enter text in '" + control.controlDescription + "' textbox due to reason: " + e);
    }
}
...
public async selectFromDropdownByValue(control: WebControl, optionTobeSelcted: string) {
    try {
        cy.get(control.selector).select(optionTobeSelcted);
...
*/
public async selectDropDownByIndex(control: WebControl, indexOfOption: number) {
    try {
        cy.get(control.selector).select(indexOfOption);
    logAction("Selected '" + indexOfOption + "' from '" + control.controlDescription + "'");
    } catch (error) {
        errorLog("'Unable to select '" + indexOfOption + "' from '" + control.controlDescription + "'");
    }
}
...
public async selectCheckbox(control: WebControl, expectedCheckboxValue: boolean = true) {
    if (expectedCheckboxValue == true) {
        cy.get(control.selector).first().check().then(() => {
            logAction("Set checkbox value of '" + control.controlDescription + "' as checked");
        });
    }
    else {
        cy.get(control.selector).first().uncheck().then(() => {
...
public async scrollToControl(control: WebControl) {
    cy.get(control.selector).scrollIntoView();
}
